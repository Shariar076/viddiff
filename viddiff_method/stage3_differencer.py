...
class Differencer:
    ...
    def __init__(
        self, args, args_logging, dataset, videos, proposals, retrieved_frames
    ):
        self.args = args
        ...
        self.results_subdir = Path(args_logging.results_dir) / "stage3_differencer"
        self.results_subdir.mkdir(exist_ok=True, parents=True)

        self.sample_key_to_differences_gt = {
            item["sample_key"]: item["differences_gt"] for item in self.dataset
        }
    ...
    def prepare_prompts(self):
        ...
        for sample, video0, video1 in zip(self.dataset, self.videos0, self.videos1):
            ...
            for difference_idx in retrieve_frames_0.keys():
                ...
                query_string = proposal.differences[difference_idx]["query_string"]
                prompt = prompt.replace("{action}", proposal.action_description)
            ...
            self.prompts[sample_key]["difference_idxs"] = prompts_difference_idxs
    ...
    def call_vlm(self):
        ...
        start = time.time()
        json_mode = True
        vlm_results_batch = call_gpt_batch(
            texts=batch_texts, imgs=batch_frames, json_mode=json_mode, **kwargs_gpt
        )
    ...
    def log_vlm_call(self, log_imgs, i, difference_idx, key, imgs, text, msg, action):
        """
        ...
        assert type(difference_idx) is str, "expect one difference per vlm call"
