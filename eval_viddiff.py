...
def eval_viddiff(
    ...
    predictions_unmatched = validate_prediction_schema(
        predictions_unmatched, n_differences, eval_mode
    )

    if eval_mode == "open" and not diffs_already_matched:
        # do LLM-based matching
        predictions, predictions_excess = do_matching(
            dataset, predictions_unmatched, seed
        )
...
def log_input_preds(dataset, predictions_unmatched, results_dir):
    log_preds = {row["sample_key"]: v for row, v in zip(dataset, predictions_unmatched)}
...
def log(dataset, df, metrics, predictions_excess, results_dir):
    ...
    if predictions_excess is not None:
        predictions_excess_dict = {
            row["sample_key"]: pred for (row, pred) in zip(dataset, predictions_excess)
        }
...
def validate_prediction_schema(
    predictions_unmatched: list[dict], n_differences: list[int], eval_mode
):
    ...
    if eval_mode == 0:
        for i, preds in enumerate(predictions_unmatched):
            if len(preds) > n_differences[i]:
                raise ValueError(
                    f"Maximum number of allowed differences is {n_differences[i]} "
...
def do_matching(dataset, predictions_unmatched, seed):
    ...
    for row, pred_unmatched in zip(dataset, predictions_unmatched):
        ...
        differences_gt = {
            k: v for k, v in row["differences_gt"].items() if v is not None
        }
        ...
        prompt = prompt.replace("{action_description}", row["action_description"])
        diff_description_gt = {
            k: v["description"]
            for k, v in row["differences_annotated"].items()
            if k in keys_keep
        }
        diff_description_pred = {k: v["description"] for k, v in pred_unmatched.items()}
        prompt = prompt.replace("{differences0}", json.dumps(diff_description_gt))
        prompt = prompt.replace("{differences1}", json.dumps(diff_description_pred))
    ...
    for i, (row, differences_gt, pred_unmatched, match) in enumerate(
        ...
        for k, v in match.items():
            pred[k] = {}
            pred[k]["gt_description"] = row["differences_annotated"][k]["description"]
    ...
    for pred_unmatched, pred in zip(predictions_unmatched, predictions):
        ...
        pred_excess = {
            k: v for k, v in pred_unmatched.items() if k not in pred_keys_matched
        }
...
def add_gt_and_details(dataset, predictions):
    ...
    for sample, pred in zip(dataset, predictions):
        ...
        keys_gt = [key for key, value in differences_gt.items() if value is not None]
        for key in keys_gt:
            if key not in pred:
...
def test_reverse_statements(predictions, seed, batch_size):
    ...
    for pred in predictions:
        for k, v in sorted(pred.items(), key=lambda x: int(x[0])):
            ...
            if pred[k]["pred_description"] is None:
                continue
            statements.append([pred[k]["gt_description"], pred[k]["pred_description"]])
            idx += 1
    ...
    for i in range(0, len(statements), batch_size):
        ...
        prompt = prompt.replace("{statements}", str(statements[i : i + batch_size]))
        batch_prompts_text.append(prompt)
    ...
    if not all(val in {"0", "1"} for val in is_opposite):
        raise ValueError(
            f"LLM issue in `test_reverse_statements`. The LLM out "
    ...
    if len(is_opposite) != len(statements):
        raise ValueError(
            f"LLM issue in `test_reverse_statements`. Array has the wrong "
...
def eval_mcq_ab(dataset, predictions, results_dir):
    ...
    df = pd.DataFrame(
        results, columns=["sample_key", "gt", "pred", "action", "name", "description"]
    )
