...
def load_viddiff_dataset(
    splits=["easy"], subset_mode="0", cache_dir=None, test_new=False
):
    """
    ...
    def _map_elements_to_json(example):
        ...
        example["differences_annotated"] = json.loads(example["differences_annotated"])
...
def _clean_annotations(example):
    ...
    differences_gt_labeled = {
        k: v for k, v in example["differences_gt"].items() if v is not None
    }
...
def get_video_data(
    videos: dict, cache=True, cache_dir="cache/cache_data", overwrite_cache=False
):
    """
    ...
    for i in [0, 1]:
        ...
        if cache:
            ...
            if os.path.exists(memmap_filename) and not overwrite_cache:
                video_info = np.load(
                    f"{memmap_filename}.info.npy", allow_pickle=True
                ).item()
                video = np.memmap(
                    memmap_filename,
                ...
                video_dict["fps"] = video_dict[
                    "fps_original"
                ]  # since we don't downsample here
                video_dicts.append(video_dict)
                continue

        is_dir = Path(path).is_dir()
        if is_dir:
            video = _load_video_from_directory_of_images(path, frames_trim=frames_trim)
        ...
        if cache:
            np.save(
                f"{memmap_filename}.info.npy",
                {"shape": video.shape, "dtype": video.dtype},
            )
            memmap = np.memmap(
                memmap_filename, dtype=video.dtype, mode="w+", shape=video.shape
            )
            memmap[:] = video[:]
            memmap.flush()
            video = memmap
...
def _load_video(f, return_fps=True, frames_trim: slice = None) -> np.ndarray:
    ...
    if len(vid) > 50000:
        raise ValueError("Video probably has too many frames to convert to a numpy")
...
def _subsample_video(
    video: np.ndarray, fps_original: int, fps_target: int, fps_warning: bool = True
):
    """
    ...
    if subsample_time < 1 and fps_warning:
        logging.warning(
            f"Trying to subsample frames to fps {fps_target}, which "
    ...
    if fps_new != fps_target and fps_warning:
        logging.warning(
            f"Config lmm.fps='{fps_target}' but the original fps is {fps_original} "
